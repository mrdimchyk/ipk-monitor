...
class Scraper:
    def __init__(self, db_path="ipk.sqlite3", timeout=30, delay=0.4):
        ...
        self.session.headers.update({
            "User-Agent": "Mozilla/5.0 (compatible; IPK-Digest/1.1)",
    ...
    def parse_listing(self, html, page_url):
        ...
        for a in soup.select('a[href*="/view/"]'):
            ...
            cells = [
                self.clean(c.get_text(" ", strip=True))
            ...
            # VOBU currently exposes the document number as link text.
            link_text = self.clean(a.get_text(" ", strip=True))
            num_match = IPK_RE.search(link_text) or IPK_RE.search(row_text)
            number = num_match.group(0) if num_match else link_text

            # Listing columns are date / number / issuer / rubric.
            issuer = cells[2] if len(cells) > 2 else ""
            category = cells[3] if len(cells) > 3 else ""

            path_tail = urlparse(href).path.rstrip("/").split("/")[-1]
            source_id = path_tail.split("-", 1)[0]

            out.append(Record(
                url=href,
                source_id=source_id,
                number=number,
                ipk_date=self.iso_date(date_match.group(1)),
                issuer=issuer,
                category=category,
            ))
        return out
    ...
    def parse_detail(self, html, rec):
        ...
        # The current VOBU detail pages expose the document as readable page
        # text. Remove navigation/review/footer noise where possible.
        ...
        # Drop obvious site chrome around the actual document.
        filtered = []
        for line in lines:
        ...
        if not rec.number:
            m = IPK_RE.search(rec.title + "\n" + rec.content)
            if m:
                rec.number = m.group(0)
        return rec

    def exists(self, url):
        return self.db.execute(
            "SELECT 1 FROM ipk WHERE source_url=?", (url,)
    ...
    def save(self, rec):
        ...
        self.db.execute("""
        INSERT INTO ipk (
          source_url, source_id, number, ipk_date, issuer, category,
          title, content, content_hash, first_seen_at, last_seen_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(source_url) DO UPDATE SET
          source_id=excluded.source_id,
          number=excluded.number,
          ipk_date=excluded.ipk_date,
          issuer=excluded.issuer,
          category=excluded.category,
          title=excluded.title,
          content=CASE WHEN excluded.content <> '' THEN excluded.content
                       ELSE ipk.content END,
          content_hash=CASE WHEN excluded.content <> '' THEN excluded.content_hash
                            ELSE ipk.content_hash END,
          last_seen_at=excluded.last_seen_at
        """, (
            rec.url, rec.source_id, rec.number, rec.ipk_date, rec.issuer,
            rec.category, rec.title, rec.content, digest, now, now
        ))
        self.db.commit()
    ...
    def collect(self, max_pages=20, overlap_days=7):
        ...
        for page in range(1, max_pages + 1):
            ...
            oldest = min(date.fromisoformat(r.ipk_date) for r in records)
...
def digest_html(records):
    ...
    if not records:
        ...
        <h2>ІПК — {today}</h2><p>Нових ІПК не знайдено.</p>
        </body></html>"""

    groups = {}
    for r in sorted(records, key=lambda x: (x.ipk_date, x.number), reverse=True):
        groups.setdefault(r.category or "Без рубрики", []).append(r)

    parts = [
        "<!doctype html><html><body>",
        f"<h2>Нові ІПК — {today}</h2>",
        f"<p><b>Кількість нових документів: {len(records)}</b></p>"
    ]
    ...
    for category, items in groups.items():
        ...
        for r in items:
            parts.append(
                "<li>"
                f"<b>{escape(r.ipk_date)}</b> — "
                f"<a href=\"{escape(r.url)}\">{escape(r.number)}</a>"
                f"<br><small>{escape(r.issuer)}</small>"
                "</li>"
            )
        parts.append("</ul>")
...
def digest_text(records):
    ...
    lines = [f"Нових ІПК: {len(records)}", ""]
    for r in sorted(records, key=lambda x: (x.ipk_date, x.number), reverse=True):
        lines.append(f"{r.ipk_date} | {r.number} | {r.url}")
    return "\n".join(lines)
...
def main():
    ...
    new_records = scraper.collect(args.max_pages, args.overlap_days)
